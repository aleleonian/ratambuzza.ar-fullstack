...
async function deletePostcard(db, postcardId) {
  ...
  const data = await getPostcardImages(db, postcardId);

  if (data.image_url) {
    try {
      const filePath = path.join(process.cwd(), 'public', data.image_url.replace(/^\/+/, ''))
      await fsPromises.unlink(filePath);
    }
    catch (error) {
      console.error("deletePostcard() error: " + error);
    }
  }

  if (data.thumbnail_url) {
    try {
      const filePath = path.join(process.cwd(), 'public', data.thumbnail_url.replace(/^\/+/, ''))
      await fsPromises.unlink(filePath);
    }
    catch (error) {
      console.error("deletePostcard() error: " + error);
...
async function updateOnlyJobStatus(db, result = {}) {
...
async function getTripMembersAvatars(db, tripId) {
  const [rows] = await db.execute(`
    SELECT u.handle, u.id, u.avatar_head_file_name, u.avatar_file_name FROM trip_members 
...
async function getPostcardImages(db, postcardId) {
  const [rows] = await db.execute(`
    SELECT p.image_url, p.thumbnail_url FROM postcards p 
    WHERE p.id = ?
  `, [postcardId]);
  return rows[0];
}

module.exports = { getPostcardImages, deletePostcard, getTripMembersAvatars, insertPostcard, getNextPendingJob, markJobInProgress, updateJobStatus, markJobComplete, markJobFailed, updateOnlyJobStatus }